...
A simplified summary of the fund manager's logic and behaviours in specific order:

* It first checks for a general market panic, that is, it monitors for an *active liquidity shock* exogenous event in its data feed and de-risks if one is found.  
* If there are no *active liquidity shocks* to consider, the manager will continue on with its examination of the historical correlation between government deficits (aka, the *fiscal balance*) and the central bank's interest (base) rate response. If it predicts a rate hike in response to a higher than normal current deficit, it will actively sell its long-term bonds to protect its portfolio.  
* If there is no crisis (liquidity shock event) and no clear prediction, it falls back to a simple interest-rate trend-following strategy.
...
[^1]:  The Domestic Stalwarts: Pension Funds and Insurers \- The Accidental Vigilantes