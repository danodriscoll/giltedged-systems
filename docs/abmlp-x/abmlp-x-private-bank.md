...
* The bank acts as the *primary-dealer* for all government debt, that is, it will purchase the long-term bonds and treasury bills issued by the debt management office (DMO). In short, it will pay for each new tranche of debt instruments (assets) by creating new deposits for the government in equal amounts (the liabilities). Subsequently, the bank will sell its portfolio of government debt instruments on to the fund manager for a small, high-volume *primary dealer spread*.
...
* The bank is the source of all private credit in the system. It creates new deposits (money) whenever it issues a loan. The bank currently issues two types of loan. It earns a profit on the *spread* between its *base rate* and the profile sensitive (variable) rate it will charge on these loans:  
  ...
  * The bank's `process_loan_repayments` method is the authoritative method that manages the repayment process, tracks missed payments, and calculates and records the interest income for itself and the interest expense for its clients.
...
* Last, but by no means least, the bank is responsible for initiating all default and firm bankruptcy proceedings in the model. If the bank's own equity falls below zero, it will request a bailout from the central bank.
