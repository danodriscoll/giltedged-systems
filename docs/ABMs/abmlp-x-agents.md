...
* The **debt management office (DMO)**: It acts as the Treasury's debt operations layer within the model. Rather than simply issuing debt whenever a deficit appears, the DMO converts financing requirements into paced market issuance, selecting between long-term bonds and T-bills while shaping maturity structure and smoothing supply across time. This allows debt creation to behave more like an operational funding process than an instantaneous fiscal reflex, with the private bank acting as the model's primary dealer and first absorber of new issuance.
